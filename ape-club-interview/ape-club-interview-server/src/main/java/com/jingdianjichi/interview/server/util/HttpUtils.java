    ...
    public static String executePost(String url, String reqArgs,
        ...
        try {
            ...
            String responseBody = response.getEntity() != null
                    ? getStreamAsString(response.getEntity().getContent(), "UTF-8")
                    : "";
            if (statusCode == HttpStatus.SC_OK || statusCode == HttpStatus.SC_CREATED) {
                result = responseBody;
            } else {
                log.error("HttpClientUtil.executePost.error.status:{},url:{},body:{}", statusCode, url, responseBody);
                throw new IllegalStateException("HTTP请求失败，状态码=" + statusCode);
            }
        } catch (Exception e) {
            log.error("HttpClientUtil.executePost.error:{},url:{}", e.getMessage(), url, e);
        }
        return result;
    }

...
import com.jingdianjichi.circle.api.enums.AnswerStatusEnum;
import com.jingdianjichi.circle.api.enums.CompleteStatusEnum;
import com.jingdianjichi.circle.api.enums.IsDeletedFlagEnum;
import com.jingdianjichi.circle.api.enums.SubjectInfoTypeEnum;
import com.jingdianjichi.circle.api.req.*;
import com.jingdianjichi.circle.api.vo.*;
...
//                deleteCharAt 方法来删除字符串缓冲区中最后一个字符
...
//            插入练题记录
    ...
    /**
     * 交卷时获取到这次交卷时所作答的答题情况，把没有作答的补充到数据库，
     * 同时更新practice_set表中的热度，
     * practice_info中的正确率，完成状态
     * @param req
     * @return
     */
    @Override
    //propagation:
    // 指定事务的传播行为。常见的值有：
    // REQUIRED（默认值）：如果当前存在事务，则加入该事务；否则，创建一个新的事务。
    ...
    public Boolean submit(SubmitPracticeDetailReq req) {
         ...
         * 这行代码将 correctCount 除以 totalCount。
         * 4 表示结果保留四位小数。
         * BigDecimal.ROUND_HALF_UP 是一种四舍五入模式，表示当小数点后第五位大于等于 5 时向上进位，否则舍去。
         */
        ...
        List<Long> subjectIds = practiceDetailPOList.stream()
                .map(PracticeDetailPO::getSubjectId)
                .collect(Collectors.toList());

        List<PracticeSetDetailPO> minusList = practiceSetDetailPOS.stream()
                // contains(item.getSubjectId()) 检查 item.getSubjectId()
                //是否在收集到的 subjectId 列表中。
                //! 取反，保留那些 subjectId 不在 practiceDetailPOList 中的元素。
                .filter(item -> !subjectIds.contains(item.getSubjectId()))
                .collect(Collectors.toList());
...
//        判断题，我要知道错误是对的还是正确是对的
    ...
    public ReportVO getReport(GetReportReq req) {
        //试卷题目，正确题目数，技能图谱List<ReportSkillVO>

...
public class ShareCircleController {
    ...
    public Result<Boolean> saveCircle(@RequestBody SaveShareCircleReq req) {
        try {
            if (log.isInfoEnabled()) {
            ...
            if (Objects.nonNull(req.getParentId()) && req.getParentId() != -1) {
                ...
                Preconditions.checkNotNull(circle, "父级不存在");
            }

            Boolean result = shareCircleService.saveCircle(req);

            if (log.isInfoEnabled()) {
                log.info("新增圈子：{}", result);
            }
            return Result.ok(result);
            ...
            log.info("新增圈子失败：{}", e.getMessage(), e);
